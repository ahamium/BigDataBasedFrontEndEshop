...
const ProductForm = (props) => {
  ...
  //const [richDescription, setRichDescription] = "richDescription";
  ...
  const addProduct = () => {
    ...
    if (item !== null) {
      axios
        .put(`${baseURLS}products/${item.id}`, formData, config)
        .then((res) => {
      ...
      axios
        .post(`${baseURLS}products`, formData, config)
        .then((res) => {
          if (res.status == 200 || res.status == 201) {
            Toast.show({
              topOffset: 60,
